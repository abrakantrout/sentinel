  ...
  return (
    <div className="p-6 md:p-8 bg-slate-950 min-h-screen text-slate-100 font-sans space-y-8 select-none">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl flex flex-col justify-between space-y-4 relative">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-800/80">
            <div>
              <div className="flex items-center gap-2.5">
                ...
                {connectionStatus === 'LIVE' && isChartLive ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-950 text-emerald-400 border border-emerald-600/50 text-[10px] font-mono font-bold">
                    <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                    LIVE · LAST {windowSize} TXS
                  </span>
                ) : !isChartLive ? (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-950 text-amber-400 border border-amber-600/50 text-[10px] font-mono font-bold">
                    <Pause className="w-3 h-3 fill-amber-400" />
                    PAUSED ({chartBufferQueue.length})
                  </span>
                ) : (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-rose-950 text-rose-400 border border-rose-600/50 text-[10px] font-mono font-bold">
            ...
            <div className="flex items-center gap-2 font-mono text-xs">
              <button
                type="button"
                onClick={() => {
                  if (isChartLive) setIsChartLive(false);
                  else handleResumeLive();
                }}
                className={`px-3 py-1 rounded-lg font-bold flex items-center gap-1.5 transition-all ${
                  isChartLive ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-emerald-600 text-white shadow-lg'
                }`}
              >
                {isChartLive ? <Pause className="w-3.5 h-3.5" /> : <Play className="w-3.5 h-3.5 fill-white" />}
                <span>{isChartLive ? 'PAUSE' : 'RESUME LIVE'}</span>
              </button>
                ...
                {[10, 20, 30, 50].map((sz) => (
                  <button
                    ...
                    className={`px-2 py-0.5 rounded font-bold transition-all ${
                      windowSize === sz ? 'bg-sky-600 text-white' : 'text-slate-400 hover:text-slate-200'
                    }`}
                  >
                    {sz}TX
                  </button>
          ...
          {/* Compact Live Metric Header Bar */}
          <div className="grid grid-cols-4 gap-3 p-3 bg-slate-950/80 rounded-xl border border-slate-800 font-mono text-xs">
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-bold block">CURRENT RISK</span>
              <span className={`text-base font-extrabold ${
                liveMetrics.currentRisk >= 85 ? 'text-rose-400' : liveMetrics.currentRisk >= 70 ? 'text-amber-400' : liveMetrics.currentRisk >= 40 ? 'text-sky-400' : 'text-emerald-400'
              }`}>
                {liveMetrics.currentRisk}
              </span>
            </div>
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-bold block">AVG RISK</span>
              <span className="text-base font-extrabold text-slate-200">{liveMetrics.avgRisk}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-bold block">HIGH RISK</span>
              <span className="text-base font-extrabold text-amber-400">{liveMetrics.highRiskCount}</span>
            </div>
            <div>
              <span className="text-[10px] text-slate-500 uppercase font-bold block">CRITICAL</span>
              <span className="text-base font-extrabold text-rose-400">{liveMetrics.critRiskCount}</span>
            </div>
          </div>

          {/* STREAMING RECHARTS AREA & LINE CHART WITH RISK AREA GLOW */}
          <div className="h-[250px] w-full mt-2">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={liveTrendWindow}>
                <defs>
                  {/* Solid Rich SENTINEL Blue Area Gradient (Reference #1A3F75) - NO FADING TO TRANSPARENT */}
                  <linearGradient id="sentinelSolidBlueGradient" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor="#1E40AF" stopOpacity={0.88} />
                    <stop offset="50%" stopColor="#1A3F75" stopOpacity={0.78} />
                    <stop offset="100%" stopColor="#0F2B5C" stopOpacity={0.68} />
                  </linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" strokeOpacity={0.4} vertical={false} />
                <XAxis dataKey="timestamp" stroke="#64748b" fontSize={10} fontFamily="JetBrains Mono" />
                <YAxis domain={[0, 100]} ticks={[0, 25, 50, 75, 100]} stroke="#64748b" fontSize={10} fontFamily="JetBrains Mono" />
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px', color: '#f8fafc' }}
                  itemStyle={{ fontSize: '11px', fontFamily: 'JetBrains Mono' }}
                />

                {/* Solid Rich SENTINEL Blue Area Fill */}
                <Area 
                  type="monotone" 
                  dataKey="score" 
                  stroke="none" 
                  fill="url(#sentinelSolidBlueGradient)" 
                  isAnimationActive={false} 
                />

                {/* Subtle Blue Glow Line Underlay */}
                <Line 
                  type="monotone" 
                  dataKey="score" 
                  stroke="#38bdf8" 
                ...
                {/* Bright Crisp SENTINEL Cyan/Blue Risk Curve Line */}
                <Line 
                  type="monotone" 
                  dataKey="score" 
                  name="Risk Score" 
            ...
            <div className="flex flex-wrap gap-2 text-xs font-mono">
              {recentRiskEvents.map((evt) => (
                <button
                  key={evt.tx_id}
                  type="button"
                  onClick={() => setSelectedChartPoint(evt)}
                  className={`px-2.5 py-1 rounded-lg border flex items-center gap-2 transition-all ${
                    evt.score >= 85 ? 'bg-rose-950/80 text-rose-300 border-rose-600/60' :
                    evt.score >= 70 ? 'bg-amber-950/80 text-amber-300 border-amber-600/60' :
                    evt.score >= 40 ? 'bg-sky-950/80 text-sky-300 border-sky-600/60' :
                    'bg-slate-900 text-slate-300 border-slate-800'
                  }`}
                >
                  <span className="font-extrabold">{evt.score}</span>
                  <span className="text-slate-400">{evt.tx_id}</span>
                </button>
              ))}
            </div>
          </div>

          {/* SELECTED POINT TRANSACTION INSPECTOR MODAL */}
          {selectedChartPoint && (
            <div className="absolute top-16 right-6 z-40 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl text-slate-100 font-mono text-xs space-y-3 w-80 backdrop-blur-md">
              <div className="flex justify-between items-center pb-2 border-b border-slate-800">
                <span className="font-bold text-sky-400 uppercase tracking-wider">TRANSACTION INSPECTOR</span>
                <button type="button" onClick={() => setSelectedChartPoint(null)} className="text-slate-500 hover:text-slate-200">
                  <X className="w-4 h-4" />
                </button>
              </div>

              <div className="space-y-1.5">
                <div className="flex justify-between"><span className="text-slate-400">TX ID:</span> <span className="font-bold">{selectedChartPoint.tx_id}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Timestamp:</span> <span>{selectedChartPoint.timestamp}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Risk Score:</span> <span className="font-bold text-rose-400">{selectedChartPoint.score}</span></div>
                <div className="flex justify-between"><span className="text-slate-400">Risk Level:</span> <span className="font-bold text-amber-400">{selectedChartPoint.risk_level}</span></div>
                {selectedChartPoint.rawTx && (
                  <>
                    <div className="flex justify-between"><span className="text-slate-400">Amount:</span> <span className="font-bold text-emerald-400">₹{Number(selectedChartPoint.rawTx.amount || 0).toLocaleString('en-IN')}</span></div>
                    <div className="flex justify-between"><span className="text-slate-400">Channel:</span> <span>{selectedChartPoint.rawTx.channel || 'UPI'}</span></div>
                    <div className="flex justify-between"><span className="text-slate-400">Sender:</span> <span>{selectedChartPoint.rawTx.sender_account_id || 'ACC-SRC'}</span></div>
                    <div className="flex justify-between"><span className="text-slate-400">Receiver:</span> <span>{selectedChartPoint.rawTx.receiver_account_id || 'ACC-REC'}</span></div>
                  </>
                )}
              </div>
            </div>
